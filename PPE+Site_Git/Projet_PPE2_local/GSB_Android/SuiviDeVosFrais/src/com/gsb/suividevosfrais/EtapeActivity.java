...
import android.widget.DatePicker.OnDateChangedListener; //public static interface DatePicker.OnDateChangedListener 
...
/**
 * Classe d'activité pour les frais Etapes (image avec :tasse caffee ): "activity_etape.xml"
...
public class EtapeActivity extends Activity { // extends -> étendre une classe implements -> c'est la même chose mais pour une interface 
	...
	protected void onCreate(Bundle savedInstanceState) { //désérialisation à la création de l'activité
		...
		setContentView(R.layout.activity_etape); //permet de désérialiser un fichier XML d'affichage dans une classe java, à savoir une Activity, récupère la vue
		//modification de l'affichage de la date du DatePicker : 
		Global.changeAfficheDate((DatePicker) findViewById(R.id.datEtape)); //permet de spécifier l'id
		...
		dat_clic(); //mise a jour de l'affichage de la qte en fonction de la date  
	}
	...
	private void valoriseProprietes(){
		annee = ((DatePicker) findViewById(R.id.datEtape)).getYear(); //obtenir et valoriser l'année
		mois = ((DatePicker) findViewById(R.id.datEtape)).getMonth() + 1; //obtenir et valoriser le mois ( +1 parce que ca commence a 0)
		//récupération de la qte correspondant au mois actuel
		qte = 0;
		Integer key = annee*100+mois;
		if(Global.listFraisMois.containsKey(key)){ //si la clé est ok 
			qte = Global.listFraisMois.get(key).getEtape(); // getEtape
		}
		((EditText)findViewById(R.id.txtEtape)).setText(qte.toString()); //Valorise info , changement du textEtape
	}
	...
	private void imgReturn_clic(){
		((ImageView) findViewById(R.id.imgEtapeReturn)).setOnClickListener(new ImageView.OnClickListener(){ //click sur l'imgEtapeReturn
	...
	/**
	 * Sur le clic du bouton valider : 
	 */
	private void cmdValider_clic(){
		((Button) findViewById(R.id.cmdEtapeValider)).setOnClickListener(new Button.OnClickListener(){ //click sur cmdEtapeValider
			public void onClick(View v){
				Serializer.serialize(Global.filename, Global.listFraisMois, EtapeActivity.this); //sérialisation
				retourActivityPrincipale(); //retour à l'activité principale
			}
		});
	}
	...
	/**
	 * Sur le clic du bouton plus : 
	 */
	private void cmdPlus_clic(){
		((Button)findViewById(R.id.cmdEtapePlus)).setOnClickListener(new Button.OnClickListener(){ //click sur  cmdEtapePlus
			public void onClick(View v){
	...
	/**
	 * Sur le clic du bouton moins : 
	 */
	private void cmdMoins_clic(){
		((Button)findViewById(R.id.cmdEtapeMoins)).setOnClickListener(new Button.OnClickListener(){ //click sur  cmdEtapeMoins
			public void onClick(View v){
	...
	/**
	 * Sur le changement de date : 
	 */
	private void dat_clic(){
		final DatePicker uneDate = (DatePicker)findViewById(R.id.datEtape); //recuération de la vue 
	...
	private void enregNewQte(){
		...
		Global.listFraisMois.get(key).setEtape(qte); //changement de la quantité en fonction de la clé
	}
	...
	private void retourActivityPrincipale(){
		Intent intent = new Intent(EtapeActivity.this,MainActivity.class); //changement d'activité
