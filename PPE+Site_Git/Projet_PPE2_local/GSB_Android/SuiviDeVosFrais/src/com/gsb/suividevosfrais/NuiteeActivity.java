...
/**
 * Classe d'activité pour les frais de nuits (image avec : lit ): "activity_nuitee.xml"
...
public class NuiteeActivity extends Activity { // extends -> étendre une classe implements -> c'est la même chose mais pour une interface

	//informations affichées dans l'activité : propriété privée
	...
	protected void onCreate(Bundle savedInstanceState) { //désérialisation à la création de l'activité 
		...
		setContentView(R.layout.activity_nuitee); //permet de désérialiser un fichier XML d'affichage dans une classe java, à savoir une Activity, récupère la vue
	...
	private void valoriseProprietes(){
		annee = ((DatePicker) findViewById(R.id.datNuitee)).getYear(); //obtenir et valoriser l'année
		mois = ((DatePicker) findViewById(R.id.datNuitee)).getMonth() + 1; //obtenir et valoriser le mois ( +1 parce que ca commence a 0
		//récupération de la qte correspondant au mois actuel
		qte = 0;
		Integer key = annee*100+mois;
		if(Global.listFraisMois.containsKey(key)){ //si la clé est ok 
			qte = Global.listFraisMois.get(key).getNuitee(); //getNuitee
		}
		((EditText)findViewById(R.id.txtNuitee)).setText(qte.toString()); //Valorise info , changement du textNuitee
	}
	...
	private void imgReturn_clic(){
		((ImageView) findViewById(R.id.imgNuiteeReturn)).setOnClickListener(new ImageView.OnClickListener(){ //click sur imgNuiteeReturn
	...
	private void cmdValider_clic(){
		((Button) findViewById(R.id.cmdNuiteeValider)).setOnClickListener(new Button.OnClickListener(){ //clisk sur cmdNuiteeValider
			public void onClick(View v){
				Serializer.serialize(Global.filename, Global.listFraisMois, NuiteeActivity.this); //serialistion
				retourActivityPrincipale(); //retour à l'activité principale
			}
		});
	}
	...
	private void cmdPlus_clic(){
		((Button)findViewById(R.id.cmdNuiteePlus)).setOnClickListener(new Button.OnClickListener(){ //click sur cmdNuiteePlus
			public void onClick(View v){
				qte+=1; //ajout de 1 en 1
	...
	private void cmdMoins_clic(){
		((Button)findViewById(R.id.cmdNuiteeMoins)).setOnClickListener(new Button.OnClickListener(){ //click sur cmdNuiteeMoins
			public void onClick(View v){
	...
	private void enregNewQte(){
		...
		Global.listFraisMois.get(key).setNuitee(qte); //changement de la quantité en fonction de la clé
	}
	...
	private void retourActivityPrincipale(){
		Intent intent = new Intent(NuiteeActivity.this,MainActivity.class); //changement d'activité
